...
abstract: |
  The article uses a quantitative exploratory methodology to analyze the 'Quality of Government Institute' Basic Dataset (2016). It presents the application of four multivariate models that combine two techniques of data analysis, factorial and regression, in order to verify the validity of the causal inference of these models against the assumptions indicated in the literature. At the same time, it shows how the combination of these two techniques helps the researcher to identify in a robust way the independent causal effects that affect the phenomenon of researcher interest, in our case the civil liberty in Contemporary States.  The contribution of this article to the field of multivariate models applied to the social sciences is given to shed light on the importance of verifying the ten assumptions of linear regression models, in this sense, we stated that the more robust the model applied, greater will be the validity of the causal inference that is intended to establish.
...
Furthermore, models 1 and 2 differ from models 3 and 4 in the dependent variable used to measure civil liberty. In models 1 and 2 we measured the effects of the independent variables on civil liberty as a categorical dependent variable with values varying from 1 to 7 -1 represents the countries with the highest and 7 those with the lowest civil liberty-, on the other hand, with models 3 and 4 we measured the effects of the independent variables on civil liberty as a continuous dependent variable -with values varying from 1 to 10, 1 was assigned to being the value assigned to countries with low and 10 with high levels of civil liberty-.

Thus, the article is divided into three sections, the first presents step by step the construction of the democratic inclusiveness index (IDI), composed of two latent variables [^3] named institutional efficiency and ethnic-linguistic plurality which were transformed using the technique of factorial analysis; the second section uses the two factorial indicators that make up our construct as independent variables in four models of multivariate linear regression, to which we add three more variables that we believe exert an intervening effect on our variable of interest, civil liberty; the third section summarizes the results of the four models and the fourth section presents a brief conclusive discussion.
...
```{r analyze1, echo=FALSE, message=FALSE, warning=FALSE, results='hide'} 
...
ggplot(data = melted_cormat,
  ...
  labs(title = "Graph 1. Correlation Matrix", caption = "Based on Basic Dataset from QoG (2016)") + theme(axis.text.x = element_text(angle = 90, hjust=0)) +
  geom_tile()

```

On appendix, the descriptive statistics of the independent variables ($X1... + X8$) used to compose the factors are presented in detail, tables 1 and 2 present the indicators that were used in the construction of the factorial indicators 'Institutional Efficiency' (factor 1) and 'Ethnic-linguistic Plurality' (factor 2).
...
Graph 2 shows the total variance of each factor that will be transformed, the variability of the variables and the accumulated percentage of each variable extracted for the construction of the factors. In Graph 3 the information is in a scree plot. Objectively, both of them indicate the amount of factors that should be extracted; the dashed line represents the cutting line of Kaiser rule; and, the abrupt drop signals a transformation on the independent variables into two constructs.
...
plot(pca1, main="Graph 2. 2 Factors") # cotovelo em 2 fatores
...
ggplot(analysis_data, aes(x = `Ethnic_linguistic Plurality`, y = `Institucional Efficiency`)) +
  ...
  labs(x = "Ethnic_linguistic Plurality", y = "Institutional Efficiency", 
       title = "Graph 4. Index of inclusive Democracy", caption = "Based on Basic Dataset from QoG (2016)")
...
In this sense, we can identify the analysis units by quadrant. In quadrant 1 are the countries with low institutional efficiency and high ethnic-linguistic plurality; quadrant 2 gathers the countries with high ethnic-linguistic plurality and high institutional efficiency; in quadrant 3 are the countries with low institutional efficiency and low ethnic-linguistic plurality; and, in the quadrant 4 are the countries with high institutional efficiency and low ethnic-linguistic plurality.
...
Thus, it is recommended that the researcher elaborates models capable of minimizing the information of the variables and maximizing the number of observations, attention to these procedures helps to increase the degree of reliability and the chance of generalization of the results.
...
plot(betas, y.axis, type="p", pch=19, xlab="Magnitude of Coefficients", ylab="", 
     ...
     cex=1,yaxs="i",xaxs="i", main="Graph 5. Confidence Intervals")
...
plot(betas, y.axis, type="p", pch=19, xlab="Magnitude of Coefficients", ylab="", 
     axes=F, xlim=c(min(IC-.4), max(IC+.4)), ylim=c(min(y.axis-.2), max(y.axis+.2)), 
     cex=1,yaxs="i",xaxs="i", main="Graph 6. Confidence Intervals")
segments(IC[,1], y.axis, IC[,2], y.axis)
axis(1, at=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), 
     labels=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), tick=T, cex.axis=1, 
     mgp=c(2,.7,0))
...
plot(betas, y.axis, type="p", pch=19, xlab="Magnitude of Coefficients", ylab="", 
     axes=F, xlim=c(min(IC-.4), max(IC+.4)), ylim=c(min(y.axis-.2), max(y.axis+.2)), 
     cex=1,yaxs="i",xaxs="i", main="Graph 7. Confidence Intervals")
segments(IC[,1], y.axis, IC[,2], y.axis)
axis(1, at=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), 
     labels=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), tick=T, cex.axis=1, 
     mgp=c(2,.7,0))
...
plot(betas, y.axis, type="p", pch=19, xlab="Magnitude of Coefficients", ylab="", 
     axes=F, xlim=c(min(IC-.4), max(IC+.4)), ylim=c(min(y.axis-.2), max(y.axis+.2)), 
     cex=1,yaxs="i",xaxs="i", main="Graph 8. Confidence Intervals")
segments(IC[,1], y.axis, IC[,2], y.axis)
axis(1, at=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), 
     labels=seq(round(min(IC-.9)), round(max(IC+.9)), by=0.1), tick=T, cex.axis=1, 
     mgp=c(2,.7,0))
...
plot(analysis_data$Predicted, analysis_data$Residuo, pch=21, bg="red", col="red", main="Graph 9. Residual Distribution")
abline(0,0, lty = 2)
...
plot(analysis_data_2$Predicted, analysis_data_2$Residuo, pch=21, bg="red", col="red", main="Graph 10. Residual Distribution")
abline(0,0, lty = 2)
...
```{r analyze 9, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}

par(mfrow=c(1,2))
## Non-linearity test
# component added to waste 
avPlots(model_1, main="Graph 11. Added-Variable Plots")

## Non-linearity test
# component added to waste 
avPlots(model_2, main="Graph 12. Added-Variable Plots")
...
par(mfrow=c(1,2))
hist(analysis_data$Residuo, freq=FALSE, ylim=c(0,0.6), main="Graph 13. Histogram of std errors")
xfit <- seq(min(analysis_data$Residuo),max(analysis_data$Residuo),length=40) 
yfit <- dnorm(xfit) 
lines(xfit, yfit)

# Scatter histogram of two residues model 2
hist(analysis_data_2$Residuo, freq=FALSE, ylim=c(0,0.6), main="Graph 14. Histogram of std errors")
...
The tenth and final assumption regarding the Central Limit Theorem that proposes to increase the number of cases against the estimated parameters was tested by increasing the number of cases from 51 (model 1) to 127 (model 2), removing the variable confidence in government. In the appendix, the result observed in table 10 indicates that the explanatory power of model 2 is smaller then the previous one in terms of $R-adjusted$ and degrees of freedom, therefore, in this case the increase in the number of cases had little influence on the results of the model 2.

In relation to models 3 and 4 that use the continuous dependent variable to measure civil liberty, the analysis indicates homoscedastic distribution of the residues against the predicted values. Graphs 15 and 16 summarize the uniform dispersion of the difference between the observed results and the results predicted by the models, therefore both satisfy the fourth assumption.
...
plot(analysis_data_3$Predicted, analysis_data_3$Residuo, pch=21, bg="red", col="red", main="Graph 15. Residual Distribution")
abline(0,0, lty = 2)
...
plot(analysis_data_4$Predicted
     , analysis_data_4$Residuo, pch=21, bg="red", col="red", main="Graph 16. Residual Distribution")
abline(0,0, lty = 2)
...
Again the sixth assumption that supports the absence of correlation between the independent variables and the error term can not be satisfied by all variables of models 3 and 4. As shown in graphs 17 and 18, in model 3 the distribution of the error term by individual parameter follows a heteroscedastic pattern for the variables institutional efficiency and accountability, but the distribution of the variables trust in government, favoritism in government decisions, and ethnic-religious plurality approximate a homoscedastic pattern, already in model 4, with greater number of cases and without the variable trust in government, only the variable ethnic-linguistic plurality approaches a homoscedastic distribution, the other three variables follow a heteroscedastic pattern.

```{r analyze13,echo=FALSE, message=FALSE, warning=FALSE, results='hide'}

par(mfrow=c(1,2))
## Non-linearity test
# component added to waste  
avPlots(model_3, main="Graph 17. Added-Variable Plots")

## Non-linearity test
# component added to waste 
avPlots(model_4, main="Graph 18. Added-Variable Plots")

```

Similar to models 1 and 2, the seventh and eighth assumptions of the regression models were satisfied from the exploratory analysis of the data, using the factorial analysis technique. We chose to use two latent variables as functional mechanisms to avoid bias of possible omitted variables and to condense information from highly correlated variables into a single factorial indicator.
...
hist(analysis_data_3$Residuo, freq=FALSE, ylim=c(0,0.4), main="Graph 19. Histogram of std errors")
...
hist(analysis_data_4$Residuo, freq=FALSE, ylim=c(0,0.4), main="Graph 20. Histogram of std errors")
...
Thus, the robustness tests indicated that model 1 is inferior to models 3 and 4, difference given by the use of the continuous dependent variable versus the categorical one of the first model. Based on the tests, it was identified that model 4 presented the most reliable results using the verified assumptions.

To conclude, based on the results of the four models presented in this article, we prove the importance of using the techniques of factorial analysis and linear regression to identify the causal effects that produce a certain phenomenon of interest of the researcher, in our case civil liberty. In this sense, we reaffirm that the more robust the applied model, the greater the validity of the causal inference that one intends to establish.
...
_____. (2014). Factor analysis and Linear Regression - the importance of the joint applicability of two data analysis techniques. Revista Eletrônica de Ciência Política, vol 5, n.2.

FIGUEIREDO FILHO, D. B.; SILVA JÚNIOR, J. A. da (2010). Visão além do alcance:
uma introdução à análise fatorial. Opinião Pública, v. 16, n. 1, p. 160-185.

FIGUEIREDO FILHO, D. B.; PARANHOS, R.; ROCHA, E. C. da; SILVA Jr., J. A da;
MAIA, R. (2013). Análise de componentes principais para construção de indicadores sociais. Rev.
Bras. Biom., São Paulo, v.31, n.f1, p.61-78.
...
Tabel 3 - Sample Suitability Tests 
...
```{r analyze20, eval=FALSE}
              ...
              Tabel 6 - Rotated Matrix Component (varimax) 
...
```{r analyze21, eval=FALSE}
           ...
           Tabel 7 - Mean variation of factors 
...
stargazer(model_1, model_2, model_3, model_4, float.env = "sidewaystable", type = "text", title = "Model Results", style = "apsr", align=TRUE, no.space=TRUE, p.auto=FALSE)
